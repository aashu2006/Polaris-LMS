...
const SessionFeedbackModal: React.FC<SessionFeedbackModalProps> = ({ isOpen, onClose, session, mentorId }) => {
  ...
  // Fetch students for this session using CustomReports pattern
  useEffect(() => {
    const fetchStudents = async () => {
      ...
      try {
        ...
        // Fetch session analytics to get students list (like CustomReports does)
        const response = await api.multimedia.attendance.getSessionAnalytics(sessionId);
        const studentsData = response.data.students || [];
        
        // Filter out mentor from students list
        const filteredStudents = studentsData.filter((student: SessionStudent) => student.studentId !== mentorId);
        
        setStudents(filteredStudents);
        
        // Initialize feedback for each student (only 2 fields)
        const initialFeedbacks: Record<string, StudentFeedback> = {};
        filteredStudents.forEach((student: SessionStudent) => {
  ...
  const handleStudentFeedbackChange = (
    studentId: string, 
    field: 'performance_rating' | 'overall_performance', 
    value: string | number
  ) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNotification({ type: 'success', message: 'Session feedback submitted successfully for session and all students!' });
      setTimeout(() => {
        onClose();
      }, 2000);
    } catch (error: any) {
      console.error('❌ Error submitting session feedback:', error);
      setNotification({ type: 'error', message: `Failed to submit session feedback: ${error.message || 'Unknown error'}` });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto p-2 sm:p-4">
      <div className="bg-slate-800 text-white rounded-xl shadow-2xl w-full max-w-6xl m-0 sm:m-4 p-3 sm:p-4 md:p-6 max-h-[95vh] sm:max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-start sm:items-center gap-2 mb-4 sm:mb-6 border-b border-slate-700 pb-3 sm:pb-4">
          <h2 className="text-base sm:text-lg md:text-xl font-bold flex-1 pr-2 break-words">
            Add Session Feedback – {session.course_name || session.title || 'Session'}
          </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Areas where the session could be improved..."
                  rows={2}
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Overall session performance feedback..."
                  rows={2}
                  className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-yellow-500 placeholder-slate-500 resize-none"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Key strengths observed in the session..."
                  rows={2}
                  className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-yellow-500 placeholder-slate-500 resize-none"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Homework assigned during the session..."
                  rows={2}
                  className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-yellow-500 placeholder-slate-500 resize-none"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Assignments given during the session..."
                  rows={2}
                  className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-yellow-500 placeholder-slate-500 resize-none"
                />
              </div>
          ...
          */}
          ...
          <div className="bg-slate-700/30 rounded-lg p-4 sm:p-5 border border-slate-600">
            ...
            {loadingStudents ? (
              ...
              <div className="text-center py-8 text-slate-400">No students found for this session</div>
            ) : (
              <div className="space-y-4">
                {students.map((student) => (
                  <div key={student.studentId} className="bg-slate-700/50 rounded-lg p-3 sm:p-4 border border-slate-600">
                    ...
                    {/* Student Feedback Form - Only 2 Fields */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                      {/* Performance Rating */}
                      <div>
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          placeholder="Describe student performance..."
                          rows={2}
                          className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-yellow-500 placeholder-slate-500 resize-none"
                        />
                      </div>
